...
class DecisionMamba(nn.Module, GenerationMixin):
    def __init__(self, state_size : int, action_size : int, d_model : int, n_layer : int,  initializer_cfg = None, device = None, dtype = None):
        ...
        self.predict_return = torch.nn.Linear(d_model, 1)
    ...
    def forward(self, states, actions, rewards, returns_to_go, position_ids=None, inference_params=None, num_last_tokens=0):
        """
...
class ElasticDecisionMamba(nn.Module):
    def __init__(
        self,
        state_dim,
        act_dim,
        ...
        blocks = [
            MambaBlock(ModelArgs(d_model=h_dim))
            for _ in range(n_blocks)
        ]
        self.mamba_blocks = nn.Sequential(*blocks)
